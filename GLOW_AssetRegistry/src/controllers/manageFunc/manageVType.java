...
public class manageVType implements manageInterface{

	@Override
	public void addToDB(String input) {
		try {
			new Database().insertValueType(input);
		} catch (Exception e) {
	...
	public void editToDB(Reference oldReference, Reference newReference) {
		try {
			new Database().updateValueType(oldReference, newReference);
		} catch (Exception e) {
	...
	public ArrayList<Reference> getListFromDB() {
		try {
			return new Database().getAllValueType();
		} catch (Exception e) {
